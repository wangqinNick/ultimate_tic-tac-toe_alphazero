...
def move(board,action, player):
    ...
    bestPosition.append(int (remainder/3))  # row
    bestPosition.append(remainder%3)  # col

    # place piece at position on board
    board[bestPosition[0]][bestPosition[1]][bestPosition[2]] = turn  # mark as 'X' or 'O'

    emptyMiniBoard = [[" "," "," "], [" "," "," "], [" "," "," "]]  # ?

    wonBoard = False
    win = False
    mini = board[bestPosition[0]]  # sub_board (2D list)
    subBoard = bestPosition[0]  # sub_board number
    x = bestPosition[1]  # row on sub_board
...
# take in the opponent's input (row, col) and returns a integer consists of sub_board, row and col value
def human_turn(board,subBoard,turn):
    ...
    if subBoard == 9 or board[subBoard][1][1] == "x" or board[subBoard][1][1] == "o" or len(possible) > 9:
        ...
        sub_r = next_r % 3  # row number on the sub_board
        sub_c = next_c % 3  # col number on the sub_board
        
    return subBoard * 9 + sub_r * 3 + sub_c
